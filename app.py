...
portfolio_size = st.sidebar.number_input("Portfolio Capital (₹)", value=1000000, step=50000)
...
# =============================================================
    ...
    if st.button("⬅️ Change Stock Selection"):
        go_to_page(1)
        st.rerun()
    ...
    if failed:
        st.warning(f"Could not load data for: {', '.join(failed)}. Verify their ticker symbols.")
 
# =============================================================
# PAGE 3: Deep-Dive Chart & Plan
# =============================================================
elif st.session_state.page == 3:
    stock_name = st.session_state.active_stock
    ticker = DEFAULT_WATCHLIST.get(stock_name, f"{stock_name}.NS")
    ...
    if df is not None:
        ...
        rsi = round(df['RSI'].iloc[-1], 2)
        sma_20 = round(df['SMA_20'].iloc[-1], 2) if not pd.isna(df['SMA_20'].iloc[-1]) else cmp
        sma_50 = round(df['SMA_50'].iloc[-1], 2) if not pd.isna(df['SMA_50'].iloc[-1]) else round(cmp * 1.05, 2)
        sma_200 = round(df['SMA_200'].iloc[-1], 2) if not pd.isna(df['SMA_200'].iloc[-1]) else round(cmp * 1.10, 2)
        ...
        t2 = sma_200
        t3 = round(cmp + (risk_per_share * 3.0), 2)
        rr_ratio = round((t2 - cmp) / (risk_per_share + 1e-5), 2)
 
        # Metric Cards
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("CMP", f"₹{cmp}")
        m2.metric("Stop Loss", f"₹{sl}", f"-₹{risk_per_share}")
        m3.metric("Target 1 (50 DMA)", f"₹{t1}", f"+{round(((t1-cmp)/cmp)*100, 1)}%")
        m4.metric("Position Size", f"{position_qty} Shares", f"Risk: ₹{max_risk_inr:,.0f}")
        ...
        fig.update_layout(
            title=f"{stock_name} Daily Chart with 20, 50 & 200 DMAs",
            xaxis_rangeslider_visible=False,
            template="plotly_dark",
            height=500,
            margin=dict(l=20, r=20, t=40, b=20)
        ...
        # Detail sections using clean write blocks
        tab1, tab2, tab3 = st.tabs(["📋 Execution Setup", "📊 Fundamentals", "⚠️ Invalidation Rules"])
        
        with tab1:
            st.markdown("### Actionable Trading Setup")
            st.write(f"* **Entry Range:** ₹{round(cmp * 0.99, 2)} – ₹{cmp}")
            st.write(f"* **Stop-Loss:** ₹{sl} (1% below 52W low of ₹{low_52w})")
            st.write(f"* **Target 1 (50 DMA):** ₹{t1} (Book 40%)")
            st.write(f"* **Target 2 (200 DMA):** ₹{t2} (Book 35%)")
            st.write(f"* **Target 3 (Swing High):** ₹{t3} (Book 25%)")
            st.write(f"* **Risk-to-Reward Ratio:** 1 : {rr_ratio}")
            st.write(f"* **Allocated Capital:** ₹{capital_outlay:,.2f}")
        ...
        with tab2:
            ...
            summary_text = info.get('longBusinessSummary', 'No description available.')
 
            st.write(f"* **Market Cap:** {mcap}")
            st.write(f"* **Trailing P/E:** {pe_val}")
            st.write(f"* **Price to Book:** {pb_val}")
            st.write(f"* **14-Day RSI:** {rsi}")
